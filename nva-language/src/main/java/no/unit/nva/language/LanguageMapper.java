...
public final class LanguageMapper {
  ...
  public static Language getLanguageByIso6391Code(String code) {
    return (Language)
        mappedValue(
            ALL_LANGUAGES.stream()
                .filter(
                    language -> convertString(code).equalsIgnoreCase(language.getIso6391Code())));
  }
    ...
    return (Language)
        mappedValue(
            ALL_LANGUAGES.stream()
                .filter(
                    language ->
                        language.getIso6392Code().stream()
  ...
  public static Language getLanguageByIso6393Code(String code) {
    return (Language)
        mappedValue(
            ALL_LANGUAGES.stream()
                .filter(
                    language -> convertString(code).equalsIgnoreCase(language.getIso6393Code())));
  }

  public static LanguageDescription getLanguageByEnglishName(String name) {
    return mappedValue(
        ALL_LANGUAGES.stream()
            .filter(language -> convertString(name).equalsIgnoreCase(language.getEng())));
  }

  public static LanguageDescription getLanguageByBokmaalName(String name) {
    return mappedValue(
        ALL_LANGUAGES.stream()
            .filter(language -> convertString(name).equalsIgnoreCase(language.getNob())));
  }

  public static LanguageDescription getLanguageByNynorskName(String name) {
    return mappedValue(
        ALL_LANGUAGES.stream()
            .filter(language -> convertString(name).equalsIgnoreCase(language.getNno())));
  }

  public static LanguageDescription getLanguageBySamiName(String name) {
    return mappedValue(
        ALL_LANGUAGES.stream()
            .filter(language -> convertString(name).equalsIgnoreCase(language.getSme())));
  }

  public static LanguageDescription getLanguageByUri(URI uri) {
    return mappedValue(
        ALL_LANGUAGES.stream().filter(language -> uri.equals(language.getLexvoUri())));
  }
  ...
  public static LanguageDescription getLanguageByPotentialIsoCode(String potentialIsoCode) {
    return mappedValue(
        ALL_LANGUAGES.stream().filter(language -> matchesCode(potentialIsoCode, language)));
  }
  ...
  private static LanguageDescription replaceMiscellaneous(LanguageDescription language) {
    return MISCELLANEOUS.equals(language) ? MULTIPLE : language;
  }
